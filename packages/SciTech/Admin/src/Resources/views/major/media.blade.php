        ...
        <div class="col-12">
            <div class="card card-secondary">
                <div class="card-header">
                    <h3 class="card-title">สร้างข้อมูลโปสเตอร์</h3>
                </div>
                <div class="card-body p-3">
                    {!! Form::open(['route' => ['admin.major.media.store', 'major' => $major->id], 'autocomplete' => 'off', 'id' => 'gallery', 'class' => 'dropzone']) !!}
                    ...
                    <ul class="upload-preview">
                        <div class="overflow-lists">
                            @foreach($major->getMedia('posters') as $mediaItem)
                                <li class="dz-preview">
                                    <div class="dz-image">
                                    ...
                                    <div class="dz-action">
                                        @if(!$mediaItem->hasCustomProperty('thumbnail'))
                                            <a class="dz-thumbnail" href="{{ route('admin.major.media.thumbnail', ['major' => $major, 'id' => $mediaItem->id]) }}">ตั้งเป็นรูปตัวอย่าง</a>
                                        @endif

                                        <a class="dz-remove" href="{{ route('admin.major.media.destroy', ['major' => $major, 'id' => $mediaItem->id]) }}" data-dz-remove>ลบ</a>
                                    </div>
                                </li>
                            @endforeach
                        </div>
        ...
        Dropzone.options.gallery = {
            ...
            init:function() {
                this.on("success", function(file, response) {
                    ...
                    var url_thumbnail = "/admin/major/" + {{ $major->id }} + "/media/" + response.id + "/thumbnail";
                    $(el).append('<a class="dz-thumbnail" href="' + url_thumbnail + '">ตั้งเป็นรูปตัวอย่าง</a>');

                    var url_delete = "/admin/major/" + {{ $major->id }} + "/media/" + response.id + "/delete";
